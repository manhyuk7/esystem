...
static struct mutex my_mutex1;    // Mutex 선언
static struct mutex my_mutex2;    // Mutex 선언
...
static int thread_function1(void *arg) {
    ...
    for (i = 0; i < 5; i++) {
        // Mutex 획득
        printk(KERN_INFO "Thread %s is trying to Lock mutex1\n", (char *)arg);
        if (mutex_lock_interruptible(&my_mutex1)) {
            pr_err("Failed to acquire mutex in thread %s\n", (char *)arg);
            return -1;
        }
        printk(KERN_INFO "Thread %s Locked mutex1\n", (char *)arg);

        // 잠시 대기
        msleep(1000);       

       // Mutex 획득
        printk(KERN_INFO "Thread %s is trying to Lock mutex2\n", (char *)arg);
        if (mutex_lock_interruptible(&my_mutex2)) {
            pr_err("Failed to acquire mutex in thread %s\n", (char *)arg);
            return -1;
        }
        printk(KERN_INFO "Thread %s Locked mutex2\n", (char *)arg);
       ...
       // GPIO 18: LED 끄기
        ...
        printk(KERN_INFO "Thread %s released the mutex1, mutex2\n", (char *)arg);
...
static int __init mutex_module_init(void) {
  ...
  // GPIO 핀 설정 (예: GPIO 23을 출력으로 설정)
    ...
    printk(KERN_INFO "Mutex module initialized\n");
...
// 메모리 매핑 해제
    ...
    printk(KERN_INFO "Mutex module exited\n");
}
